...
global.rolePowMover.run = function (creep) {
    ...
    var power_spawns = creep.room.find(FIND_STRUCTURES).filter((structure) => structure.structureType == STRUCTURE_POWER_SPAWN);
    var power_spawns_pow = power_spawns.filter((structure) => structure.structureType == STRUCTURE_POWER_SPAWN && structure.store.getUsedCapacity(RESOURCE_POWER) < 20);
    var power_spawns_en = power_spawns.filter((structure) => structure.structureType == STRUCTURE_POWER_SPAWN && structure.store.getFreeCapacity(RESOURCE_ENERGY) > 0);
