...
global.roleHandler = {
    ...
    run: function (creep) {
        ...
        if (!creep.memory.moving) {
            ...
            if (creep.room.memory.mainTower != undefined) {
                // console.log("main tower found")
                if(creep.room.memory.mainTower.enemyInRoom != undefined && creep.room.memory.mainTower.enemyInRoom == true) {
                    var towers = creep.room.find(FIND_STRUCTURES).filter((structure) => {
                        return structure.structureType == STRUCTURE_TOWER && structure.store.getFreeCapacity(RESOURCE_ENERGY) > 200 && !Memory.rooms[creep.memory.baseRoomName].creeps.handlers.currentTargets.includes(structure.id);
                    });
                    if (towers.length) {
                        Log(creep, 6);
            ...
            if (creep.memory.currentTarget != null) {
                ...
                if (currentTarget == null || currentTarget.store.getFreeCapacity(RESOURCE_ENERGY) == 0) {
                    ...
                    if (currentTarget.structureType == STRUCTURE_SPAWN && Memory.rooms[creep.room.name].mainSpawn.refilling != false && Memory.rooms[creep.room.name].mainSpawn.refilling != creep.name) {
                        // should be removed if room.creeps.handlers.currentTargets works
                        creep.memory.currentTarget = null;
                    } else {
                        for (const resourceType in creep.store) {
            ...
            var targets = creep.room.find(FIND_STRUCTURES).filter((structure) => {
                return (
                    ...
                    !Memory.rooms[creep.memory.baseRoomName].creeps.handlers.currentTargets.includes(structure.id)
                );
            });
            Log(creep, targets);
            ...
            if (!targets.length) {
                ...
                targets = creep.room.find(FIND_STRUCTURES).filter((structure) => {
                    return structure.structureType == STRUCTURE_TOWER && structure.store.getFreeCapacity(RESOURCE_ENERGY) > 0 && !Memory.rooms[creep.memory.baseRoomName].creeps.handlers.currentTargets.includes(structure.id);
                });
            }
