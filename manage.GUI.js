...
drawGUI = function () {
    ...
    for (var room in Game.rooms) {
        try {
            ...
            new RoomVisual().text(
                `🔋  ExcessEnergy: ${creepRoomMap.get(r.name + "eenergy")}  Recent: ${Memory.rooms[r.name].stats.energyLevels.averageRecent} Average: ${Memory.rooms[r.name].stats.energyLevels.average} ${
                    Memory.rooms[r.name].stats.energyLevels.averageRecent > creepRoomMap.get(r.name + "eenergy") ? "❌" : "✅"
                }`,
                1,
                listOffset + inc(),
                { align: "left", font: fontSize }
            );
            ...
            _.forEach(Memory.rooms[r.name].sources, (s) => {
                ...
                text =
                    `    ${source.room.name}:${source.id.substr(-3)}: H: ${creepRoomMap.get(r.name + "harvesterTarget" + source.id) || 0} ` +
            ...
            _.forEach(Memory.rooms[r.name].externalSources, (s) => {
                ...
                if (source == undefined) return;
                text =
                    `    ${source.room.name}:${source.id.substr(-3)}: H: ${creepRoomMap.get(r.name + "harvesterExtTarget" + source.id) || 0} ` +
                    `${Memory.rooms[source.room.name].sources[source.id].currentMiningParts || 0}/${Memory.rooms[source.room.name].sources[source.id].targetMiningParts || 0} ` +
