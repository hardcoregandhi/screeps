creepReduction = function (r) {
    ...
    _.forEach(Memory.rooms[r.name].sources, (s) => {
        ...
        _.forEach(s.targettedByList, (creepName) => {
            creepMiningParts = Game.creeps[creepName].body.reduce((previous, p) => {return p.type == WORK ? previous += 1 : previous;}, 0);
        ...
        if (s.container != undefined) {
            ...
            _.forEach(s.container.targettedByList, (creepName) => {
                creepCarryParts = Game.creeps[creepName].body.reduce((previous, p) => {return p.type == CARRY ? previous += 1 : previous;}, 0);
